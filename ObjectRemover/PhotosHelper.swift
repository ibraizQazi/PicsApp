...
public struct PhotosHelper {
    ...
    public static func createAlbum(named: String, completion: @escaping (PHAssetCollection?) -> ()) {
     ...
     */
    public static func getAlbum(named name: String, completion: @escaping (PHAssetCollection?) -> Void) {
        DispatchQueue.global(qos: .background).async {
            ...
            if let album = collections.firstObject {
                ...
                PhotosHelper.createAlbum(named: name) { album in
                    DispatchQueue.main.async {
                        completion(album)
                    }
                }
     ...
     */
    public static func saveImage(image: UIImage, toAlbum named: String, completion: ((Bool, Error?) -> Void)? = nil) {
        PhotosHelper.getAlbum(named: named) { album in
    ...
    /**
     ...
     - parameter completion: Called in the background when the image was saved or in case of any error.
     */
    public static func saveImage(image: UIImage, to album: PHAssetCollection, completion: ((Bool, Error?) -> Void)? = nil) {
        DispatchQueue.global(qos: .background).async {
            PHPhotoLibrary.shared().performChanges({
                let assetRequest = PHAssetChangeRequest.creationRequestForAsset(from: image)
                guard let placeholder = assetRequest.placeholderForCreatedAsset else { completion?(false, nil); return; }
     ...
     */
    public static func getImagesFromAlbum(named: String, options: PHImageRequestOptions = PHImageRequestOptions(), fetchOptions: FetchOptions = FetchOptions(), completion: @escaping (AssetFetchResult<UIImage>) -> ()) {
        DispatchQueue.global(qos: .background).async {
    ...
    /**
     ...
     - parameter completion:   Called in the background when images were retrieved or in case of any error.
     */
    public static func getImagesFromAlbum(album: PHAssetCollection, options: PHImageRequestOptions = defaultImageFetchOptions, fetchOptions: FetchOptions = FetchOptions(), completion: @escaping (AssetFetchResult<UIImage>) -> ()) {
        DispatchQueue.global(qos: .background).async {
            PhotosHelper.getAssetsFromAlbum(album: album, fetchOptions: fetchOptions) { result in
                switch result {
                case .Asset: ()
                ...
                case .Assets(let assets):
                    ...
                    assets.forEach { asset in
                        imageManager.requestImage(for: asset, targetSize: fetchOptions.size ?? CGSize(width: asset.pixelWidth, height: asset.pixelHeight), contentMode: .aspectFill, options: options) { image, _ in
     ...
     */
    public static func getAssetsFromAlbum(album: PHAssetCollection, fetchOptions: FetchOptions = FetchOptions(), completion: @escaping (AssetFetchResult<PHAsset>) -> ()) {
        DispatchQueue.global(qos: .background).async {
...
//            let indexes = IndexSet(integersIn: range.toRange()!)
     ...
     */
    public static func getAlbums(completion: @escaping ([PHAssetCollection]) -> ()) {
        DispatchQueue.global(qos: .background).async {
            let fetchOptions = PHFetchOptions()
            ...
            [albums, smartAlbums].forEach { collections in
    ...
    /**
     ...
     - parameter completion: Called in the background when all albums were retrieved.
     */
    public static func getUserCreatedAlbums(completion: @escaping ([PHAssetCollection]) -> ()) {
        DispatchQueue.global(qos: .background).async {
            let fetchOptions = PHFetchOptions()
            fetchOptions.sortDescriptors = [NSSortDescriptor(key: "localizedTitle", ascending: true)]
     ...
     */
    public static func getCameraRollAlbum(completion: @escaping (PHAssetCollection?) -> ()) {
        DispatchQueue.global(qos: .background).async {
